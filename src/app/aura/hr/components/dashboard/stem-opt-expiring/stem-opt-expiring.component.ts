...
@Component({
  selector: 'app-stem-opt-expiring',
  templateUrl: './stem-opt-expiring.component.html',
  styleUrls: ['./stem-opt-expiring.component.scss']
})
export class StemOptExpiringComponent implements OnInit {
  ...
  countsDataGridsPanelConfig: DetailsConfig = {
    ...
    columns: [
      ...
      {
        headerDisplay: 'Stem OPT Start Date',
        key: 'issueDate',
      },
      {
        headerDisplay: 'Stem OPT End Date',
        key: 'expiryDate',
      },
    ],
    tableData: null,
  ...
  ngOnInit(): void {
    this.getStemOPTExpiringDatails();
  }
  ...
  getStemOPTExpiringDatails(): void {
    this.hrDashboardService
      .getStemOPTExpiringData(
        this.dataGridType,
        this.searchString,
